...
function schedule(step: number, bar: number, time: number) {
  ...
  } else if (th.style === 'rumba') {
    // rumba catalana: walking bass, the offbeat "ventilador" guitar strum, and palmas
    const chord = CHORDS[th.prog[bar % th.prog.length]];
    if (step === 0) bass(ctx, master, time, chord[0], spb() * 1.4, 0.5 * g);
    if (step === 3) bass(ctx, master, time, chord[0] + 7, spb() * 1.0, 0.32 * g);
    if (step === 4) bass(ctx, master, time, chord[0], spb() * 1.2, 0.42 * g);
    if (step === 6) bass(ctx, master, time, chord[2], spb() * 1.0, 0.3 * g);
    const strum = [1, 2, 3, 5, 6, 7];
    if (strum.includes(step)) {
      const accent = (step === 3 || step === 6) ? 0.3 : 0.16;
      pluck(ctx, master, time, chord[1], spb() * 0.7, accent * g);
      pluck(ctx, master, time, chord[2], spb() * 0.7, accent * 0.8 * g);
      if (step === 3 || step === 6) pluck(ctx, master, time, chord[3], spb() * 0.7, accent * 0.7 * g);
    }
    if ((step === 2 || step === 5 || step === 7) && rnd(bar * 11 + step * 3) > 0.42) {
      marimba(ctx, master, time, sc[Math.floor(rnd(bar * 7 + step * 5) * sc.length)] + 12, spb() * 1.2, 0.18 * g);
    }
    if (noise) {
